...
async def stream_end_handler(_, update: Update):
    ...
    try:
        ...
        if Queues.is_empty(chat_id):
            ...
            afk = Queues.get(chat_id)["Dosya"]
            finxx = f"{afk[0]}{afk[1]}{afk[2]}"
            got_queue = get_queue.get(chat_id)
            if got_queue:
                got_queue.pop(0)
            aud = 0
            if str(finxx) != "olmamış":
                ...
                finaltext = await app.send_photo(
                    ...
                    caption=(
                        f"🎥<b>__Çalmaya Başladı:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={afk}) \n👤**__Tarafından talep edildi:__** {mention}"
                    ),
                )
                os.remove(thumb)
                videoid = afk
                ...
                finaltext = await app.send_photo(
                    ...
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=f"🎥<b>__Çalmaya Başladı:__</b> {title} \n👤<b>__Tarafından istendi:__ </b> {mention}",
                )
            await start_timer(
                videoid,
                duration_min,
